...
void indexfinger_stretching() {
 ...
 while ((delta_alpha <= alpha_max && delta_alpha >= alpha_min)&&(delta_beta <= beta_max && delta_beta >= beta_min)&&(delta_gamma <= gamma_max && delta_gamma >= gamma_min)){
    ...
    delay(500);
    /*
    Serial.println("stretch indexfinger: delta_alpha + alpha_max + goal_alpha_min");
    Serial.println(delta_alpha);
    Serial.println(alpha_max);
    Serial.println(alpha_min);
    Serial.println("stretch indexfinger: delta_beta + beta_max + goal_beta_min");
    Serial.println(delta_beta);
    Serial.println(beta_max);
    Serial.println(beta_min);
    Serial.println("stretch indexfinger: delta_gamma + gamma_max + goal_gamma_min");
    Serial.println(delta_gamma);
    Serial.println(gamma_max);
    Serial.println(gamma_min);
    Serial.println("stretch indexfinger: x_current_4");
    Serial.println(x_current_4);
    Serial.println("stretch indexfinger: x_current_3");
    Serial.println(x_current_3);
    ...
    delay(200);
    if((x_current_1 > SERVOMAX_1 && x_current_1 < SERVOMIN_1) && (x_current_2 > SERVOMAX_2 && x_current_2 < SERVOMIN_2) && (x_current_3 > SERVOMAX_3 && x_current_3 < SERVOMIN_3) && (x_current_4 > SERVOMAX_4 && x_current_4 < SERVOMIN_4)){
      Serial.println("if condition in Stretching get TRUE");
      break;
    }
  }
}
...
void indexfinger_flexion(int percentage) {
    int delta_alpha = 0;                // corresponds to the DIP joint (fingertip).
    int delta_beta = 0;                // corresponds to the PIP joint.
    int delta_gamma = 0;               // corresponds to the MCP joint.
    ...
    int delta = alpha_max - alpha_min;
    delta = delta * percentage / 100;
    int goal_alpha_min = alpha_min + delta;

    delta = beta_max - beta_min;
    delta = delta * percentage / 100;
    int goal_beta_min = beta_min + delta;

    delta = gamma_max - gamma_min;
    delta = delta * percentage / 100;
    int goal_gamma_min = gamma_min + delta;

 while ((delta_alpha <= alpha_max && delta_alpha >= goal_alpha_min)&&(delta_beta <= beta_max && delta_beta >= goal_beta_min)&&(delta_gamma <= gamma_max && delta_gamma >= goal_gamma_min)){
    void neutral_position_wrist();
    double delta_x4 = - delta_gamma * r_c ;
    double delta_x3 = (-delta_beta * r_b) + (delta_gamma * r_c); 
    double delta_x2 = (-delta_alpha * r_a) + (delta_beta * r_b) + (delta_gamma * r_c);
    ...
    delay(500);
    Serial.println("flex indexfinger: delta_alpha + alpha_max + alpha_min");
    Serial.println(delta_alpha);
    Serial.println(alpha_max);
    Serial.println(goal_alpha_min);
    Serial.println("flex indexfinger: delta_beta + beta_max + beta_min");
    Serial.println(delta_beta);
    Serial.println(beta_max);
    Serial.println(goal_beta_min);
    Serial.println("flex indexfinger: delta_gamma + gamma_max + gamma_min");
    Serial.println(delta_gamma);
    Serial.println(gamma_max);
    Serial.println(goal_gamma_min);
    Serial.println("stretch indexfinger: x_current_4");
    Serial.println(x_current_4);
    Serial.println("stretch indexfinger: x_current_3");
    Serial.println(x_current_3);
    ...
    delay(200); 
    if((x_current_1 > SERVOMAX_1 && x_current_1 < SERVOMIN_1) && (x_current_2 > SERVOMAX_2 && x_current_2 < SERVOMIN_2) && (x_current_3 > SERVOMAX_3 && x_current_3 < SERVOMIN_3) && (x_current_4 > SERVOMAX_4 && x_current_4 < SERVOMIN_4)){
      Serial.println("if condition in flexion get TRUE");
      break;
    }
  }
}
...
//______________________________________________________________
int getBaseline() {
  Serial.println("Goint to start measurment in 5 Seconds");
  delay(5000);
  Serial.println("Starting to measure baseling");
  double baseline_sum = 0;
  for(int i = 0; i < 3000; i++){
    // read the input on analog pin 0:
    baseline_sum += analogRead(A0);
  }
  Serial.println("Ended measurement");
  Serial.println("summed value & baseline");
  Serial.println(baseline_sum);
  Serial.println(baseline_sum / 3000);
  return baseline_sum / 3000;
}

int getCurrentStatus( int baseline, int measurement){
  return (2 * ((baseline-measurement)));
}
...
indexfinger_stretching();
delay(2000);
Serial.println("flexion 50%");
indexfinger_flexion(50);
delay(2000);
Serial.println("stretching");
indexfinger_stretching();
delay(2000);
Serial.println("flexion 100%");
indexfinger_flexion(100);
delay(2000);
*//*
int sensorvalue = analogRead(A0);
int sensorvalue2 = analogRead(A1);
//Serial.println("the logarithmic stuff");
Serial.print(getCurrentStatus(baseline, sensorvalue));
Serial.print("\t");
